...
function H.entity(id, x, z)
    ...
    function e:HasTag(tag)
        return self.tags[tag] == true
    end
    function e:GetPosition()
        local xx, _, zz = self.Transform:GetWorldPosition()
        return { x = xx, y = 0, z = zz }
    end
    function e:GetCurrentPlatform()
        return self.platform
    end
    function e:GetDisplayName()
        return self.prefab
    end
    ...
    function e:RemoveEventCallback(event, fn)
        if self.events[event] then
            self.events[event][fn] = nil
        end
    end
    function e:PushEvent(event, data)
        for fn in pairs(self.events[event] or {}) do
            fn(self, data)
        end
    end
    function e:Remove()
...
function H.item(id, x, count)
    ...
    function stack:Put(other)
        if e.prefab ~= other.prefab or e.skinname ~= other.skinname then
            return
        end
        local amount = math.min(self.max - self.size, other.components.stackable.size)
        self.size = self.size + amount
        other.components.stackable.size = other.components.stackable.size - amount
        if other.components.stackable.size == 0 then
            other:Remove()
        end
    end
