...
local function run()
    ...
    if not n then world.Tick = original; print("[WW_LOAD_COMPLETE]"); return end
    samples, entities = {}, {}
    ...
    for _ = 1, n do
        ...
        e.Physics:Stop(); e.components.inventoryitem:SetLanded(true)
        e:PushEvent("on_loot_dropped")
        entities[#entities + 1] = e
    end
    G.TheWorld:DoTaskInTime(20, function()
        table.sort(samples)
        local function percentile(q) return samples[math.max(1, math.ceil(#samples * q))] or 0 end
        local total, stacks = 0, 0
        for _, e in ipairs(entities) do
            if e:IsValid() then total = total + e.components.stackable:StackSize(); stacks = stacks + 1; e:Remove() end
        end
        print(string.format("[WW_LOAD] count=%d conserved=%s stacks=%d samples=%d p50_ms=%.4f p95_ms=%.4f p99_ms=%.4f elapsed=%.2f pending=%d",
            n, tostring(total == n), stacks, #samples, percentile(.5), percentile(.95), percentile(.99), G.GetTime() - begin,
            world.items.tail - world.items.head + 1))
        next_scene = next_scene + 1; run()
    end)
end
run()
