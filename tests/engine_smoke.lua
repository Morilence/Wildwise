...
local function test(name_, fn)
    ...
    if ok then
        passes = passes + 1
        print("[WW_ENGINE_PASS] " .. name_)
    else
        failures = failures + 1
        print("[WW_ENGINE_FAIL] " .. name_ .. " " .. tostring(message))
    end
end
local function spawn(prefab, dx, dz)
    local e = G.SpawnPrefab(prefab)
    assert(e, "missing prefab " .. prefab)
    local portal
    for _, candidate in pairs(G.Ents) do
        if candidate.prefab == "multiplayer_portal" then
            portal = candidate
            break
        end
    end
...
test("world service loaded on dedicated server", function()
    assert(world and G.TheNet:IsDedicated())
end)
test("server does not load UI", function()
...
test("food estimates match actual engine eating", function()
    ...
    viewer.components.health:SetPercent(0.5)
    viewer.components.hunger:SetPercent(0.5)
    viewer.components.sanity:SetPercent(0.5)
    local expected = Facts.viewer(food, viewer, ctx)
    local health, hunger, sanity =
        viewer.components.health.currenthealth, viewer.components.hunger.current, viewer.components.sanity.current
    assert(viewer.components.eater:Eat(food))
    assert(math.abs(viewer.components.health.currenthealth - health - expected.food_health) < 0.05)
    assert(math.abs(viewer.components.hunger.current - hunger - expected.food_hunger) < 0.05)
    assert(math.abs(viewer.components.sanity.current - sanity - expected.food_sanity) < 0.05)
end)
test("native stack count conservation", function()
    local a, b = spawn("twigs", 15, 0), spawn("twigs", 16, 0)
    a.components.stackable:SetStackSize(5)
    b.components.stackable:SetStackSize(7)
    world.items:mark(a, "world", G.GetTime())
    world.items:mark(b, "world", G.GetTime())
    world.items:process(a, G.GetTime() + 1)
    world.items:process(b, G.GetTime() + 1)
...
test("manual and unknown items protected as merge targets", function()
    ...
    world.items:mark(a, "manual", G.GetTime())
    world.items:mark(b, "world", G.GetTime())
    world.items:process(a, G.GetTime() + 1)
    world.items:process(b, G.GetTime() + 1)
...
test("observer healthdelta produces delta and releases listeners", function()
    ...
    local observer = Observer.new(ctx, function(_, _, kind, data)
        sent[#sent + 1] = { kind = kind, data = data }
    end)
    ...
    observer:tick(G.GetTime() + 0.1)
    assert(sent[2].kind == "delta")
    observer:forget(viewer)
    assert(next(observer.entities) == nil)
    observer:close()
end)
test("native MapExplorer isolation", function()
    local entity = G.CreateEntity()
    cleanup[#cleanup + 1] = entity
    entity.entity:AddTransform()
    entity.entity:AddMapExplorer()
...
test("persistent map round trip", function()
    ...
    local id = map:discover(
        "tester",
        { key = "a", prefab = "wormhole", x = 1, z = 2 },
        { key = "b", prefab = "wormhole", x = 3, z = 4 },
        false
    )
    local restored = Map.new("test")
    assert(restored:load(map:save()))
...
test("native partial inventory pickup preserves leftover stack", function()
    ...
    service:mark(item, "world", G.GetTime())
    service:process(item, G.GetTime() + 1)
...
test("native mount facts follow saddle and obedience values", function()
    ...
    cow.components.rideable:SetSaddleable(true)
    cow.components.rideable:SetSaddle(viewer, saddle)
    cow.components.domesticatable:DeltaObedience(0.75 - cow.components.domesticatable:GetObedience())
    ...
    cow.components.domesticatable:DeltaObedience(0.1)
    local renewed = Facts.common(cow, ctx)
    assert(math.abs(renewed.ride_time - G.GetTaskRemaining(cow._bucktask)) < 0.02)
