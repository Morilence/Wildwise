...
local portal
for _, e in pairs(G.Ents) do if e.prefab == "multiplayer_portal" then portal = e; break end end
local origin = portal:GetPosition()
local pig = G.SpawnPrefab("pigman"); pig.Transform:SetPosition(origin.x + 30, 0, origin.z + 30)
local a = pig.components.lootdropper:SpawnLootPrefab("flint")
local b = pig.components.lootdropper:SpawnLootPrefab("flint")
local player = G.SpawnPrefab("wilson"); player.Transform:SetPosition(origin.x + 45, 0, origin.z + 30)
local manual = G.SpawnPrefab("flint"); player.components.inventory:GiveItem(manual)
...
G.TheWorld:DoTaskInTime(6, function()
    ...
    local function test(name_, fn)
        ...
        if ok then passed = passed + 1; print("[WW_EVENT_PASS] " .. name_)
        else failed = failed + 1; print("[WW_EVENT_FAIL] " .. name_ .. " " .. tostring(message)) end
    end
    test("real lootdropper events land and merge with count conservation", function()
        local count = (a:IsValid() and a.components.stackable:StackSize() or 0) + (b:IsValid() and b.components.stackable:StackSize() or 0)
        assert(count == 2); assert(a:IsValid() ~= b:IsValid(), "drops have not merged")
    end)
    ...
    for _, e in ipairs({ a, b, manual, pig, player }) do if e:IsValid() then e:Remove() end end
