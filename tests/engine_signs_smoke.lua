...
return function(overrides)
    ...
    for prefab, enabled in pairs(overrides or {}) do
        expected[prefab] = enabled
    end
    local created, records, passed, failed = {}, {}, 0, 0
    local function result(prefab, ok, message)
        if ok then
            passed = passed + 1
            print("[WW_SIGN_PASS] " .. prefab .. " expected=" .. tostring(expected[prefab]))
        else
            failed = failed + 1
            print("[WW_SIGN_FAIL] " .. prefab .. " " .. tostring(message))
        end
    end
        ...
        for prefab, record in pairs(records) do
            local ok, message = pcall(function()
                ...
                for _, entity in pairs(G.Ents) do
                    if
                        entity.prefab == "minisign"
                        and entity.entity:GetParent() == record.container
                        and not entity.persists
                    then
                        assert(not sign, "duplicate helper signs")
                        sign = entity
                    end
                end
                assert((sign ~= nil) == expected[prefab], "helper sign presence does not match type switch")
                if sign then
                    assert(sign.components.drawable:GetImage() == record.item.prefab, "wrong inventory icon")
                end
            end)
