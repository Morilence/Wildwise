...
local function config()
    return Config.copy(Config.defaults)
end
H.test("hostility follows actual targets and changed follower ownership", function()
    ...
    enemy.components.combat = { target = pet }
    pet.components.follower = { leader = viewer }
...
H.test("new drops merge into older eligible stacks", function()
    ...
    local service = itemservice(nil, { b, a })
    service:mark(a, "world", 0)
    service:mark(b, "world", 0)
    service:tick(1)
    H.eq(a.components.stackable.size, 12)
    H.eq(b.valid, false)
end)
H.test("manual loaded and unknown targets stay protected", function()
    for _, source in ipairs({ "manual", "loaded", "unknown" }) do
        ...
        local service = itemservice(nil, { a, b })
        service:mark(a, source, 0)
        service:mark(b, "world", 0)
        service:tick(1)
...
H.test("no merging across boat and shore", function()
    local a, b = H.item(1, 0), H.item(2, 1)
    a.platform = {}
    local service = itemservice(nil, { a, b })
    service:mark(a, "world", 0)
    service:mark(b, "world", 0)
...
H.test("landing and active queue reservation block pickup and merging", function()
    ...
    service:mark(a, "world", 0)
    service:mark(b, "world", 0)
    b.components.inventoryitem.is_landed = false
    service:tick(1)
    assert(b.valid)
    b.components.inventoryitem.is_landed = true
    service:reserve(player, b, 1)
    service:tick(2)
    assert(b.valid)
    service:reserve(player, nil, 3)
    service:tick(4)
    H.eq(b.valid, false)
end)
H.test("partial stacks conserve count and skins", function()
    local a, b = H.item(1, 0, 38), H.item(2, 1, 5)
    local service = itemservice(nil, { a, b })
    service:mark(a, "world", 0)
    service:mark(b, "world", 0)
    service:tick(1)
    H.eq(a.components.stackable.size, 40)
    H.eq(b.components.stackable.size, 3)
    local d = H.item(3, 2, 4)
    d.skinname = "skin"
    service:mark(d, "world", 1)
    service:tick(2)
    assert(d.valid)
end)
H.test("pickup selects nearest eligible receiver with stable userid tie", function()
    local cfg, item, chosen = config(), H.item(5, 0, 4), nil
    cfg.items.pickup_allowed = true
    local a, b = H.entity(1, 1), H.entity(2, -1)
    a.userid, b.userid = "b", "a"
    for _, player in ipairs({ a, b }) do
        player.components.inventory = {
            Has = function()
                return true
            end,
            CanAcceptCount = function()
                return 4
            end,
            GiveItem = function()
                chosen = player
    ...
    local service = itemservice(cfg, { item }, { a, b }, function()
        return true
    end)
...
H.test("item work budget and queue bound resist burst", function()
    ...
    for i = 1, 3000 do
        service:mark(H.item(i, i * 10), "world", 0)
    end
    assert(service.tail <= 2049)
    assert(service.dropped > 0)
    service:tick(1)
    H.eq(service.processed, 16)
end)
H.test("marker validates type coordinates owner limit and expiry", function()
    ...
    for _ = 1, 4 do
        assert(map:add_ping("a", "danger", 0, 0, 0))
    end
    H.eq(map:add_ping("a", "danger", 0, 0, 0), nil)
    assert(map:add_ping("a", "danger", 0, 0, 60))
    assert(map:delete_ping("b", 0, true))
end)
H.test("pair identity persists and reconnect does not shuffle numbers", function()
    local map = Map.new("1")
    local a, b = { key = "a", prefab = "wormhole", x = 0, z = 0 }, { key = "b", prefab = "wormhole", x = 1, z = 0 }
    H.eq(map:discover("a", a, b, false), 1)
    local reloaded = Map.new("1")
    assert(reloaded:load(map:save()))
    H.eq(reloaded:discover("a", b, a, false), 1)
    H.eq(#reloaded:visible_pairs("b", true), 0)
    reloaded:discover("a", a, b, true)
    H.eq(#reloaded:visible_pairs("b", true), 1)
end)
H.test("changed destination invalidates old pair and preserves monotonic ID", function()
    local map = Map.new("1")
    map:discover(
        "a",
        { key = "a", prefab = "wormhole", x = 0, z = 0 },
        { key = "b", prefab = "wormhole", x = 1, z = 0 },
        true
    )
    H.eq(
        map:discover(
            "a",
            { key = "a", prefab = "wormhole", x = 0, z = 0 },
            { key = "c", prefab = "wormhole", x = 2, z = 0 },
            true
        ),
        2
    )
    H.eq(map.endpoints.b, nil)
end)
H.test("unsupported saves are preserved without silent reset", function()
    ...
    H.eq(map:load(original), false)
    H.eq(map:save(), original)
end)
H.test("exploration records only supplied authorized points and deduplicates", function()
    local map = Map.new("1")
    assert(map:record_exploration(1, 2))
    H.eq(map:record_exploration(1, 2), false)
    map:set_preference("user", "exploration", false)
    H.eq(#map.exploration, 1)
end)
local function queue()
    ...
    a.cancel = function() end
    a.progress = function()
        return "same"
    end
    a.submit = function(_, token, cb)
        a.submitted = a.submitted + 1
...
H.test("queue preserves order deduplicates and respects capacity", function()
    local q = queue()
    assert(q:add({ key = "a", prefab = "wormhole", x = 0, z = 0 }))
    H.eq(q:add({ key = "a", prefab = "wormhole", x = 0, z = 0 }), false)
    q:add({ key = "b", prefab = "wormhole", x = 1, z = 0 })
    q:add({ key = "c", prefab = "wormhole", x = 2, z = 0 })
    H.eq(q:add({ key = "d" }), false)
    H.eq(q.tasks[1].key, "a")
    H.eq(q.tasks[3].key, "c")
end)
H.test("one executor awaits one result and cancellation ignores stale callbacks", function()
    local q, a = queue()
    q:add({ key = "a", prefab = "wormhole", x = 0, z = 0 })
    q:tick(0)
    local token = q.inflight.token
    q:tick(1)
    H.eq(a.submitted, 1)
    q:cancel()
    a.callbacks[token]("done")
    H.eq(q.done, 0)
    H.eq(q.state, "idle")
end)
...
H.test("destroyed targets are skipped and merged targets redirected", function()
    local q = queue()
    local old, survivor = {}, {}
    q:add({ target = old, key = old })
    q:redirect(old, "merged", survivor)
    H.eq(q.tasks[1].target, survivor)
    q:redirect(survivor, "picked", nil)
    H.eq(#q.tasks, 0)
end)
H.test("grid uses serpentine order bounds and reprojects moving platforms", function()
    local platform = H.entity(1)
    platform.entity = {
    ...
    local points = assert(Planner.grid({ x = 10, z = 0 }, { x = 12, z = 2 }, 1, 9, function()
        return true
    end, platform))
    H.eq(#points, 9)
    H.eq(points[4].point.x, 12)
    H.eq(points[6].point.x, 10)
...
H.test("observer shares facts but isolates viewer food effects and cleans up", function()
    ...
    local obs = Observer.new({ G = { TheWorld = { state = {} } }, config = config() }, function(p, _, kind, data)
        sent[p] = { kind = kind, data = data }
    end)
    obs:subscribe(a, food, "hover", 0)
    obs:subscribe(b, food, "hover", 0)
    obs:tick(0)
    H.eq(sent[a].data.fields.food_health, 1)
    H.eq(sent[b].data.fields.food_health, 2)
    H.eq(calls, 1)
    obs:forget(a)
    assert(obs.entities[food])
    obs:forget(b)
    H.eq(next(obs.entities), nil)
...
H.test("observer rejects distance limit and expires abandoned subscriptions", function()
    ...
    H.eq(obs:subscribe(a, far, "hover", 0), false)
    assert(obs:subscribe(a, close, "health", 0))
    obs:tick(7)
    H.eq(next(obs.entities), nil)
end)
H.test("cooking returns all highest priority candidates without random choice", function()
    local cooking = {
        ingredients = { berries = { tags = { fruit = 1 } } },
        recipes = {
            cookpot = {
                a = {
                    priority = 1,
                    test = function(_, names)
                        return names.berries == 4
                    end,
                },
                b = {
                    priority = 1,
    ...
    H.eq(#result, 2)
    H.eq(result[1].name, "a")
    H.eq(result[2].name, "b")
end)
...
H.test("observer revokes data when target stops being visible", function()
    ...
    assert(obs:subscribe(a, target, "health", 0))
    obs:tick(0)
    visible = false
    obs:tick(0.1)
    assert(invalidated)
...
H.test("farm grid aligns nine points inside one native tile", function()
    ...
    local points = Planner.grid(a, b, 4 / 3, 200, function()
        return true
    end)
    H.eq(#points, 9)
    for _, point in ipairs(points) do
        assert(math.abs(point.point.x) < 2 and math.abs(point.point.z) < 2)
    end
    H.eq(Planner.bounds({ x = 0, z = 0 }, { x = 0.1, z = 0.1 }, 1, 0.5, 0.5), nil)
end)
H.test("UI has both languages for every information field and stop reason", function()
    ...
    for key in pairs(require("wildwise/services/facts").schema) do
        assert(text[key] and text[key][1] and text[key][2], key)
    end
    for _, key in ipairs({ "no_points", "inventory_full", "tool_missing", "materials_empty", "input_focus" }) do
        assert(text[key][1] and text[key][2])
    end
end)
...
H.test("item spatial index releases removed targets and respects CPU budget", function()
    ...
    service:mark(a, "world", 0)
    service:tick(1)
    assert(next(service.index))
    service:forget(a)
    H.eq(next(service.index), nil)
    local time = 0
    service.env.clock = function()
        time = time + 0.0011
        return time
    end
    for i = 2, 10 do
        service:mark(H.item(i, i * 10), "world", 1)
    end
    local before = service.processed
    service:tick(2)
    H.eq(service.processed - before, 1)
    assert(service.tail >= service.head)
end)
...
local function sign_hooks(cfg, mods)
    ...
    local api = {
        AddPrefabPostInit = function(prefab)
            registered[prefab] = true
        end,
        AddComponentPostInit = noop,
        AddPlayerPostInit = noop,
        AddModRPCHandler = noop,
        AddClientModRPCHandler = noop,
        AddShardModRPCHandler = noop,
    }
    local G = {
        KnownModIndex = {
...
H.test("container switches opt pets and food storage in while allowing chests to opt out", function()
    ...
    local hooks = sign_hooks(Config.load(function(key)
        return input[key]
    end))
...
H.test("the signs master switch and mod conflicts override every container switch", function()
    ...
    for prefab in pairs(Signs.supported) do
        H.eq(disabled[prefab], nil)
        H.eq(conflict[prefab], nil)
    end
end)
