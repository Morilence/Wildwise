...
-- 复查距离、可见性、容器所有权或当前坐骑权限。
function Observer:authorized(player, target)
    if not U.valid(target) or not U.valid(player) or not target.Transform then
        return false
    end
    ...
    if target == mount or target == player or owner == player then
        return true
    end
    ...
    if
        ...
        and (
            not U.valid(owner)
            or not owner.components
            or not owner.components.container
            or not owner.components.container:IsOpenedBy(player)
        )
    then
        return false
    end
    if U.distance(player, target) > self.context.config.observation.radius ^ 2 then
        return false
    end
...
-- 建立或续租有界订阅；同一实体只挂一个健康监听。
function Observer:subscribe(player, target, mask, now)
    if not self:authorized(player, target) then
        return false
    end
    local subscriptions = self.players[player] or {}
    self.players[player] = subscriptions
    if not subscriptions[target] and U.count(subscriptions) >= self.context.config.observation.limit then
        return false
    end
    ...
    if not record then
        ...
        record.scope:listen(target, "healthdelta", function()
            record.health_dirty = true
        end)
        record.scope:listen(target, "onremove", function()
            self:remove(target)
        end)
    end
    if subscriptions[target] then
        local sub = subscriptions[target]
        if sub.mask ~= mask then
            sub.fresh, sub.next_dynamic, record.next_common = true, 0, 0
        end
        sub.mask, sub.expires = mask, now + 6
        return true
    end
    if mask ~= "health" then
        record.next_common = 0
    end
...
function Observer:unsubscribe(player, target)
    ...
    if record then
        ...
        if next(record.subscribers) == nil then
            record.scope:close()
            self.entities[target] = nil
        end
    end
end

-- 通知订阅者目标失效，清理其全部监听与引用。
function Observer:remove(target)
    ...
    local users = {}
    for p in pairs(record.subscribers) do
        users[#users + 1] = p
    end
...
function Observer:forget(player)
    local targets = {}
    for target in pairs(self.players[player] or {}) do
...
function Observer:tick(now)
    ...
    for target, record in pairs(self.entities) do
        if not U.valid(target) then
            ...
            if refresh then
                local full = false
                for _, sub in pairs(record.subscribers) do
                    if sub.mask ~= "health" then
                        full = true
                        break
                    end
                end
                record.common = full and Facts.common(target, self.context) or Facts.health(target, self.context)
                record.next_common = now + 0.5
                self.computes = self.computes + 1
            elseif record.health_dirty then
                local hp = Facts.health(target, self.context)
            ...
            for player, sub in pairs(record.subscribers) do
                ...
                elseif sub.fresh or refresh or record.health_dirty or now >= sub.next_dynamic then
                    ...
                    if sub.mask ~= "health" and (refresh or sub.fresh or now >= sub.next_dynamic) then
                        ...
                        sub.next_dynamic = now + 0.5
                    end
                    for k, v in pairs(sub.view or {}) do
                        fields[k] = v
                    end
