...
function M.bounds(a, b, spacing, ox, oz)
    ...
    if ax > bx or az > bz then return nil end
    return { x = ax, z = az }, { x = bx, z = bz }
end
function M.grid(a, b, spacing, limit, validate, platform)
    if not U.finite(spacing) or spacing <= 0 then return nil, "invalid_spacing" end
    ...
    if nx * nz > limit then return nil, "queue_limit" end
    local points = {}
    for row = 0, nz - 1 do
        for col = 0, nx - 1 do
            ...
            if platform then record.lx, record.ly, record.lz = platform.entity:WorldToLocalSpace(point.x, 0, point.z) end
...
function M.project(record)
    if not record.platform then return record.point end
    if not U.valid(record.platform) then return nil end
...
function M.nearest(targets, start)
    ...
    for _, t in ipairs(targets) do pending[#pending + 1] = t end
    ...
    while #pending > 0 do
        ...
        for i, target in ipairs(pending) do
            ...
            if d < distance or (d == distance and target.GUID < pending[best].GUID) then best, distance = i, d end
        end
        local target = table.remove(pending, best); ordered[#ordered + 1] = target
        local tx, _, tz = target.Transform:GetWorldPosition(); x, z = tx, tz
    end
    return ordered
end
return M
