...
function M.hostility(target, viewer, scope, players, radius)
    ...
    if not U.valid(victim) then return false end
    if victim == viewer then return true, true end
    ...
    if followers and leader == viewer then return true, false end
    if nearby then
        for _, player in ipairs(players or {}) do
            if U.valid(player) and U.distance(player, viewer) <= radius * radius
                and (victim == player or (followers and leader == player)) then return true, false end
        end
    end
    return false
end
function M.select(candidates, hover, limit, now, linger)
    ...
    for target, state in pairs(candidates) do
        if U.valid(target) and state.visible and not state.dead
            and (state.hostile or now - state.last_hostile < linger) then
    ...
    table.sort(eligible, function(a, b)
        ...
        if a.state.direct ~= b.state.direct then return a.state.direct == true end
        if a.state.distance ~= b.state.distance then return a.state.distance < b.state.distance end
        return a.state.order < b.state.order
    end)
    local selected = {}
    for i = 1, math.min(limit, #eligible) do selected[#selected + 1] = eligible[i].target end
    return selected
end
return M
