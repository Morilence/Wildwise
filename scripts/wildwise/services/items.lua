...
function Items:cell(item)
    ...
    if platform and platform.entity then
        local lx, _, lz = platform.entity:WorldToLocalSpace(x, y, z)
        x, z = lx, lz
    end
...
function Items:indexitem(item)
    self:unindex(item)
    if U.call(item.components.stackable, "IsFull") then
        return
    end
...
function Items:mark(item, source, now)
    ...
    if self:sourceallowed(source) then
        self:enqueue(item, now + 0.25)
    end
end

-- 检查该来源是否被服主允许参与合堆。
function Items:stackallowed(source)
...
-- 检查来源能否进入处理队列；拾取权限不等于合堆权限。
function Items:sourceallowed(source)
    return self:stackallowed(source) or (source == "world" and self.config.items.pickup_allowed)
end

-- 加入延时处理队列；去重或超限时保留原物品。
function Items:enqueue(item, at)
    if self.queued[item] then
        return
    end
    if self.tail - self.head + 1 >= 2048 then
        self.dropped = self.dropped + 1
        return
    end
    self.tail = self.tail + 1
    self.pending[self.tail] = { item = item, at = at }
    self.queued[item] = true
end

-- 为当前拾取动作建立七秒租约；传 nil 释放租约。
...
function Items:reserved(item, now)
    for player, reservation in pairs(self.reservations) do
        if not U.valid(player) or now >= reservation.until_time then
            self.reservations[player] = nil
        elseif reservation.item == item then
            return true
        end
    end
    return false
...
-- 执行前复查来源、落地、速度、所有权、组件和预留。
function Items:eligible(item, now)
    if not U.valid(item) or not item.Transform then
        return false
    end
    local record, c = self.known[item], item.components
    if not record or not self:sourceallowed(record.source) or not c or not c.stackable or not c.inventoryitem then
        return false
    end
    if
        c.inventoryitem.owner
        or c.inventoryitem.canbepickedup == false
        or c.inventoryitem.is_landed == false
        or item:HasTag("INLIMBO")
        or item:HasTag("heavy")
        or item:HasTag("smallcreature")
        or c.health
        or c.projectile
        or c.complexprojectile
        or item:HasTag("projectile")
    ...
    if item.Physics then
        ...
        if vx * vx + vy * vy + vz * vz > 0.04 then
            return false
        end
    end
    return true
...
function Items:process(item, now)
    ...
    if cfg.items.pickup_allowed and self.known[item].source == "world" then
        ...
        table.sort(players, function(a, b)
            ...
            if ad == bd then
                return tostring(a.userid) < tostring(b.userid)
            end
            return ad < bd
        end)
        ...
        if player then
            ...
            if not U.valid(item) or c.inventoryitem.owner then
                self.env.result(item, "picked", nil)
                return
            elseif c.stackable:StackSize() < before then
                self.env.result(item, "partial", item)
            end
        end
    end
    if not self:eligible(item, now) then
    ...
    for _, target in ipairs(neighbors) do
        ...
        then
            ...
            if U.call(target.components.stackable, "IsFull") then
                self:unindex(target)
            end
            if not U.valid(item) then
                self:forget(item)
...
function Items:tick(now)
    ...
    while self.head <= stop do
        ...
        if started and self.env.clock() - started >= 0.002 then
            break
        end
        local entry = self.pending[self.head]
        self.pending[self.head] = nil
    ...
    if self.head > self.tail then
        self.head, self.tail, self.pending = 1, 0, {}
    end
end

-- 清空服务持有的引用和任务，不改变任何玩家物品。
function Items:close()
    self.pending, self.queued, self.known, self.reservations, self.index = {}, {}, {}, {}, {}
end
return Items
