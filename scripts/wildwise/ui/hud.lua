...
local HUD = G.Class(Widget, function(self, client)
    ...
    self:SetVAnchor(G.ANCHOR_BOTTOM)
    self:SetScaleMode(G.SCALEMODE_NONE)
    self.button = self:AddChild(Templates.StandardButton(function()
        client:toggle_menu()
    end, "Wildwise", { 118, 38 }))
    self.button:SetPosition(82, 55)
    self.notice = self:AddChild(Text(G.UIFONT, 24, ""))
    self.notice:SetClickable(false)
    self.queue = self:AddChild(Widget("WildwiseQueue"))
    self.queue:SetPosition(205, 170)
    self.queue_text = self.queue:AddChild(Text(G.UIFONT, 21, ""))
    self.queue_text:SetPosition(0, 46)
    self.pause = self.queue:AddChild(Templates.StandardButton(function()
        if client.queue.state == "paused" then
            client.queue:resume()
        else
            client.queue:pause()
        end
    end, client:L("pause"), { 110, 36 }))
    self.pause:SetPosition(-58, -18)
    self.clear = self.queue:AddChild(Templates.StandardButton(function()
        client.queue:cancel()
        client:clear_preview()
    end, client:L("clear"), { 110, 36 }))
    self.clear:SetPosition(62, -18)
    self.plan = self:AddChild(Widget("WildwisePlan"))
    self.plan:SetPosition(210, 270)
    self.plan_text = self.plan:AddChild(Text(G.UIFONT, 21, ""))
    self.plan_text:SetPosition(0, 32)
    self.plan_go = self.plan:AddChild(Templates.StandardButton(function()
        client:execute_plan()
    end, client:L("execute_plan"), { 135, 36 }))
    self.plan_go:SetPosition(-70, -12)
    self.plan_cancel = self.plan:AddChild(Templates.StandardButton(function()
        client:clear_preview()
    end, client:L("cancel_plan"), { 135, 36 }))
    self.plan_cancel:SetPosition(75, -12)
...
-- 按槽位复用血条控件，避免战斗中持续创建和销毁。
function HUD:bar(index)
    if self.bars[index] then
        return self.bars[index]
    end
    local bar = self:AddChild(Widget("WildwiseHealth"))
    bar:SetClickable(false)
    bar.bg = bar:AddChild(Image("images/global.xml", "square.tex"))
    bar.bg:SetSize(106, 12)
    bar.bg:SetTint(0.08, 0.06, 0.04, 0.9)
    bar.fill = bar:AddChild(Image("images/global.xml", "square.tex"))
    bar.fill:SetTint(0.78, 0.21, 0.16, 1)
    bar.text = bar:AddChild(Text(G.UIFONT, 18, ""))
    bar.text:SetPosition(0, 19)
    bar:SetClickable(false)
    self.bars[index] = bar
    return bar
end

-- 把独立信息文本绑定到当前原生 hoverer，不改写原生字符串。
function HUD:update_hover(hoverer)
    local client = self.client
    if self.hover_owner ~= hoverer then
        if self.hover_text then
            self.hover_text:Kill()
        end
        self.hover_owner, self.hover_text = hoverer, nil
    end
    if not self.hover_text then
        self.hover_text = hoverer:AddChild(Text(G.UIFONT, client.settings.info.font_size, ""))
        self.hover_text:SetColour(0.96, 0.9, 0.73, 1)
        self.hover_text:SetClickable(false)
    end
    local fields = client.hover and client.cache:get(client.hover)
    if not fields or not client.config.info.enabled or not hoverer.shown then
    ...
    self.hover_text:SetPosition(0, -85 - height / 2)
    self.hover_text:Show()
end

-- 按原生屏幕坐标刷新健康、队列、预览、骑乘和提示显示。
function HUD:OnUpdate()
    ...
    for i, target in ipairs(c.selected or {}) do
        ...
        if U.valid(target) and hp and hp.health and hp.health_max and hp.health > 0 and hp.health_max > 0 then
            ...
            if target.entity:IsVisible() and sx > 50 and sx < w - 50 and sy > 30 and sy < h - 30 then
                bar:SetPosition(sx / scale, sy / scale)
                bar:SetScale(c.settings.healthbars.scale)
                local fill = 100 * U.clamp(hp.health / hp.health_max, 0, 1)
                bar.fill:SetSize(math.max(0.01, fill), 6)
                bar.fill:SetPosition((fill - 100) / 2, 0)
                bar.text:SetString(
                    c.settings.healthbars.numbers and string.format("%.0f / %.0f", hp.health, hp.health_max) or ""
                )
                bar:Show()
                c.visible_bars[target] = true
            else
                bar:Hide()
            end
        else
    ...
    for i = #(c.selected or {}) + 1, #self.bars do
        self.bars[i]:Hide()
    end
    if c.notice and now < (c.notice_until or 0) then
        self.notice:SetPosition(w / (2 * scale), h / scale - 130)
        self.notice:SetString(c.notice)
        self.notice:Show()
    else
        self.notice:Hide()
    end
    if #c.queue.tasks > 0 then
        ...
        self.pause:SetText(c:L(c.queue.state == "paused" and "resume" or "pause"))
        self.queue:Show()
    else
        self.queue:Hide()
    end
    if c.plan_points then
        local valid = 0
        for _, record in ipairs(c.plan_points) do
            if record.valid then
                valid = valid + 1
            end
        end
        self.plan_text:SetString(c:L("enabled_points") .. ": " .. valid .. " / " .. #c.plan_points)
        self.plan:Show()
    else
        self.plan:Hide()
    end
    ...
    if mount and c.config.beefalo.enabled and c.settings.beefalo.visible then
        if self.last_mount ~= c.mount then
            self.hunger_active = (mount.hunger or 0) >= c.settings.beefalo.hunger_threshold
            self.last_mount = c.mount
        end
        if mount.hunger and mount.hunger >= c.settings.beefalo.hunger_threshold then
            self.hunger_active = true
        end
        if mount.hunger and mount.hunger <= 0 then
            self.hunger_active = false
        end
    ...
    for target, expires in pairs(c.found) do
        if not U.valid(target) or now >= expires then
            c.found[target] = nil
            if self.found[target] then
                self.found[target]:Kill()
                self.found[target] = nil
            end
        else
            local text = self.found[target]
            if not text then
                text = self:AddChild(Text(G.UIFONT, 22, ""))
                text:SetClickable(false)
                self.found[target] = text
            end
            local x, y, z = target.Transform:GetWorldPosition()
            local sx, sy = G.TheSim:GetScreenPos(x, y + 1, z)
            text:SetString("◆ " .. c:L("found"))
            text:SetPosition(sx / scale, sy / scale)
            if target.entity:IsVisible() then
                text:Show()
            else
                text:Hide()
            end
...
-- hover 文本挂在原生 hoverer 下，必须显式释放，不能只销毁本 HUD 的子控件。
-- 同时清理挂在其它控件下的 hover 文本，防止界面重建后残留。
function HUD:Kill()
    if self.hover_text then
        self.hover_text:Kill()
        self.hover_text = nil
    end
    self.hover_owner = nil
    HUD._base.Kill(self)
end

-- 按个人设置和屏蔽状态显示同世界的屏外队友位置。
function HUD:update_indicators(w, h, scale)
    ...
    if active then
        for _, record in ipairs(c.map.players or {}) do
            if
                record.userid ~= c.player.userid
                and record.shard == c.shard
                and record.x
                and not U.muted(G, record.userid)
            then
                ...
                if sx < 80 or sx > w - 80 or sy < 80 or sy > h - 80 then
                    ...
                    if not text then
                        text = self:AddChild(Text(G.UIFONT, 20, ""))
                        text:SetClickable(false)
                        self.indicators[index] = text
                    end
                    text:SetString(record.name)
                    text:SetPosition(U.clamp(sx, 80, w - 80) / scale, U.clamp(sy, 80, h - 80) / scale)
                    text:Show()
                end
            end
        end
    end
    for i = index + 1, #self.indicators do
