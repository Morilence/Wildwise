...
local function safe(value, depth, budget)
    budget.n = budget.n + 1
    if budget.n > 2048 or depth > 8 then
        return false
    end
    local kind = type(value)
    if kind == "number" then
        return U.finite(value)
    end
    ...
    for k, v in pairs(value) do
        if (type(k) ~= "string" and type(k) ~= "number") or not safe(v, depth + 1, budget) then
            return false
        end
    end
    return true
...
function M.encode(json, kind, session, sequence, data)
    ...
    if not ok or #encoded > M.MAX_BYTES then
        return nil, "payload_limit"
    end
    return encoded
end
...
function M.decode(json, bytes)
    ...
    if not ok or type(msg) ~= "table" or not safe(msg, 0, { n = 0 }) then
        return nil, "invalid_payload"
    end
    if
        msg.v ~= M.VERSION
        or type(msg.session) ~= "string"
        or #msg.session > 96
...
-- 创建以游戏时间补充的令牌桶。
function M.bucket(rate, burst, now)
    return { rate = rate, burst = burst, tokens = burst, at = now }
end

-- 扣除一次请求预算；时钟回拨不会凭空增加令牌。
function M.allow(bucket, now, cost)
    ...
    if bucket.tokens < (cost or 1) then
        return false
    end
    bucket.tokens = bucket.tokens - (cost or 1)
    return true
end

-- 比较两次字段快照，返回更新字段和被移除的键。
